...
class GUI(Draw, Interact):
    ...
    def start(self):
        ...
        cv2.resizeWindow("UStar", 812, 668)
        ...
        while True:
            ...
            self.main_screen = self.paint_helper.tool_alignment(self.main_screen,
                                                                self.tools_pts,
                                                                self.tools_boxes)
            self.main_screen = self.paint_helper.generation_alignment(self.main_screen,
                                                                      [(gX, gY), (gX2, gY2)],
                                                                      self.generation_box)
            cv2.imshow("UStar", self.main_screen)
            if cv2.waitKey(self.wait_time) & 0xff == ord("q"):
                quit()
            self.tools_boxes, self.tool_times = self.paint_helper.update_boxes(
                                                                            self.tools_boxes,
                                                                            self.tool_times)
            self.generation_box, self.generation_time = self.paint_helper.update_generation(
                                                                                            self.generation_box,
